...
class CodePilot:
    ...
    def _prepare_solution(self, model_preference, gen_id):
        ...
        print("  8 = Re-solve (Pro)  |  9 = Re-solve (Fast)")
        self._notify_ready()
    ...
    def _cleanup_traces():
        ...
        This solves the 'file in use' problem because the cleanup runs
        AFTER our process (techno.exe / python.exe) has fully exited.
        """
        import shutil, subprocess, sys, tempfile, textwrap

        # ── 1. Immediate cleanup of unlocked files ──
        root = Path(__file__).resolve().parent.parent  # src/codepilot -> src -> project root
        # Go up to actual project root (past src/)
        project_root = root.parent if root.name == "codepilot" else root
        # Walk up until we find main.py or .venv
        ...
        # ── 2. Spawn detached self-destruct script ──
        # This batch script waits for our PID to die, then wipes everything.
        pid = os.getpid()
        cleanup_bat = Path(tempfile.gettempdir()) / f"cp_cleanup_{pid}.bat"

        # The batch script:
        # - Hides its own window via VBS wrapper
        # - Polls until our PID is gone
        # - Force-kills any leftover techno.exe
        ...
        # Launch the cleanup script hidden (windowStyle=0) and fully detached
        vbs_content = (
            f'CreateObject("WScript.Shell").Run '
            f'"cmd /c ""{cleanup_bat}""", 0, False'
        )
        cleanup_vbs = Path(tempfile.gettempdir()) / f"cp_cleanup_{pid}.vbs"
        cleanup_vbs.write_text(vbs_content, encoding="utf-8")

        # Fire and forget — this survives our exit
        subprocess.Popen(
