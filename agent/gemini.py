...
class GeminiAgent:
    ...
    API_MODEL_PRO = "gemini-3.5-flash"
    ...
    def __init__(self):
        ...
        # ── Priority 1: Cookie-based GeminiDirect ──
        # Skip if API key is set (API is faster & works through firewalls)
        psid = os.getenv("GEMINI_1PSID", "").strip()
        if psid and len(psid) > 10 and not self._api_key:  # Only use cookies if no API key
            try:
                ...
                from agent.gemini_direct import GeminiDirectClient, GeminiDirectError
                self._direct_error_class = GeminiDirectError

                psidts = os.getenv("GEMINI_1PSIDTS", "").strip()
                self._direct = GeminiDirectClient(psid, psidts, timeout=self.timeout)
                self._direct.init()

                self.available_models = self._direct.available_models
                # Pick best model — exclude image-preview and tts variants
                def is_text_model(name):
                    low = name.lower()
                    return "image" not in low and "tts" not in low

                def model_version(name):
                    """Extract version for sorting (higher = better)."""
                    import re as _re
                ...
                # gemini-advanced is Google's best pro model
                pro_models = sorted(
                    [m for m in text_models if "pro" in m.lower() or "advanced" in m.lower()],
                    key=model_version, reverse=True
                )
                flash_models = sorted(
                    [m for m in text_models if "flash" in m.lower() and m not in pro_models],
                    key=model_version, reverse=True
                )
                ...
                if pro_models:
                    ...
                    self.model = all_models[0] if all_models else "gemini-2.5-flash"

                self.model_candidates = pro_models + flash_models + [m for m in text_models if m not in pro_models and m not in flash_models]
                self._closed = False

                print(f"[Gemini] Selected model: {self.model}")
                print(f"[Gemini] Mode: Direct HTTP (no gemini_webapi)")
                ...
                return  # Direct mode ready

            except Exception as exc:
                print(f"[Gemini] Direct mode failed: {exc}")
    ...
    async def _new_client(self):
        client = GeminiClient(self._psid, self._psidts, proxy=None)
        await client.init(
